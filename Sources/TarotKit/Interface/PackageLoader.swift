...
///
public class PackageLoader: Loader {
    let space: Space
    /// Graph memory into which the nodes will be imported
    var memory: GraphMemory { space.memory }
    ...
    public func load(from source: URL) throws {
        ...
        //
        do {
            package = try Package(url: source)
        }
            ...
            fatalError("Can not load package '\(source)': \(error)")
        }
        ...
        do {
            try load(package: package, model: model)
        }
        catch let error as CocoaError {
            if error.isFileError {
